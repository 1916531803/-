...
parser.add_argument("--model_path", type=str, default="./pretrained_model/model_epoch_1.pth", help="pretrained model path")
...
parser.add_argument("--test_dataset", type=str, default="SIG", help="dataset for testing")
parser.add_argument("--data_path", type=str, default="./LFData/test_RA37.h5",help="file path contained the dataset for testing")
...
def compute_quan(pred_y, target_y, ind_source, csv_name, lfi_no):
    ...
    for i in range(opt.angular_out * opt.angular_out):
        ...
        if i not in ind_source.numpy()[0]:#Harrsion
...
def test():
    ...
    with torch.no_grad():
        for k, batch in enumerate(test_loader):
            ...
            inter_lf,disp_target= model_view(ind_source, input, opt)
            # harrison
            # print(disp_target.shape,"disp_target.shape")  disp = np.float32(disp_target[0, :, :])
            # write_pfm(disp,"C:/Users/Harrison/Desktop/GroupMeeting/disp/" + "{}dsp.pfm".format(k))
