...
class Net_view(nn.Module):    
    ...
    def forward(self, ind_source, img_source, opt):
        ...
        warped_img = functional.grid_sample(warp_img_input,grid).view(N,an2,num_source,h,w) #(N,an2,num_source,h,w)
     
        return warped_img,disp
...
class Net_refine(nn.Module):    
    ...
    def forward(self, inter_lf, opt):
        ...
        ################# refine LF ###########################
        feat = self.lf_conv0(inter_lf.view(N*an2,num_source,h,w)) #[N*an2,64,h,w]
        ...
        lf = inter_lf[:,:,0,:,:] + torch.squeeze(res,2) #[N,an2,h,w] 
